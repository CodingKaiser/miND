...
if(!exists("snakemake")) {
  ...
  if(snakemake@config['includeSpikeIns'] == 1) {
    ...
    spikeInAnalysis <- snakemake@config['spikeInVersion']
  }
...
# pipeline version, read from pixi.toml (project root when rendered via snakemake)
pipelineVersion <- tryCatch({
  pixiToml <- readLines(if(exists("snakemake")) "pixi.toml" else "../pixi.toml")
  versionLine <- grep('^version', pixiToml, value = TRUE)[1]
  sub('.*"([^"]+)".*', '\\1', versionLine)
...
* Pipeline version: miND `r pipelineVersion`
...
spikeinConcentrations <- spikeInLots[["Yes"]]
...
for(i in 1:nrow(spikeins)) { # iterate over each row = sample
  ...
  if(qc.spikeInsMissing > 2) {
    ...
    if (nrow(sampleReads) == 0) {
      spikeInStats %<>% add_row(
        ID = rownames(sampleSpikeins),
        spikeInsDetected = spikeInsDetected,
        miRNAsInRange = 0, intercept = NA, slope = NA, rsq = NA,
        qc = "failed"
      )
      miRNAPredConc %<>% add_column(!!rownames(sampleSpikeins) := NA)
      miRNAPredPI %<>% add_column(!!rownames(sampleSpikeins) := NA)
    ...
    if (length(miRNAsRatioInSpikeInRange) == 0 || is.na(miRNAsRatioInSpikeInRange)) miRNAsRatioInSpikeInRange <- 0
...
if(perplexVal > 0) {
  # Rtsne aborts if any samples have identical expression vectors ("Remove
  # duplicates before running TSNE"), which happens with very low-count or
  # near-empty libraries. t-SNE is purely a visualisation aid here, so fall back
  # to a note instead of failing the whole report.
  p <- tryCatch(
    tsne(reads,
            colvec = c("violet"),
            dotsize = 3,
  ...
  if(inherits(p, "error")) {
    cat("\n\nt-SNE could not be computed for this dataset (", conditionMessage(p),
        "). This is expected when samples have identical or near-empty expression profiles.\n\n", sep = "")
  } else {
    ggplotly(p)
  }
}
...
for (i in 1:length(contrasts)) {
  ...
  if (file.exists(paste0(contrastPath, "/dePrefilteringStats.csv"))) {
    ...
    print(htmltools::tagList(xfun::embed_file(paste0(contrastPath, "/dePrefilteringRemovedGenes.csv"), text = "Download RPM table of prefiltering removed miRNAs")))
  }
